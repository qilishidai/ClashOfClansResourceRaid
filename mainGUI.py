...
def 窗口调用(调用该方法的窗口):
    ...
    本函数还有一个行为就是,将一些模拟器的状态返回到窗口类的变量中
                调用该方法的窗口.op=op
                窗口类.顶层窗口句柄=雷电模拟器运行信息[int(雷电模拟器索引)]["顶层窗口句柄"]
                调用该方法的窗口.绑定窗口句柄 = 雷电模拟器运行信息[int(雷电模拟器索引)]["绑定窗口句柄"]
    """
    ...
    while True:  #循环进攻
        ...
        if 正常结束上一轮打鱼而进入新一轮打鱼 is False:
            ...
            # if op.IsBind() == 0:
            if 未绑定过窗口:
                是否成功绑定 = op.BindWindow(雷电模拟器运行信息[int(雷电模拟器索引)]["绑定窗口句柄"], "dx2", "windows", "windows", 1)
                # 等待绑定窗口完成
                op.Delay(500)
                调用该方法的窗口.op = op
                调用该方法的窗口.顶层窗口句柄 = 雷电模拟器运行信息[int(雷电模拟器索引)]["顶层窗口句柄"]
                调用该方法的窗口.绑定窗口句柄 = 雷电模拟器运行信息[int(雷电模拟器索引)]["绑定窗口句柄"]
