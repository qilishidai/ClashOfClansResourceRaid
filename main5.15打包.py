...
while True:  #循环进攻
    ...
    op.Capture(0, 0, 2000, 2000, "aaa.bmp")
    ...
    while True:  #判断登录状态循环
        # 查找宝石数量旁边的宝石确定登录状态
        result, x, y = op.FindPic(752, 9, 789, 154, 目前脚本工作目录 + r"\img\宝石.jpg", "000000", 0.8, 0)
        # 找到了,证明已经登录,跳出循环
        if result != -1:
            break
    ...
    while True:  #本循环有两个判断,判断是否存在第二次战斗和是否出现回营按钮
        ...
        _, 回营x, 回营y = op.FindPic(0, 0, 2000, 2000, 目前脚本工作目录 + r"\img\回营.jpg", "000000", 0.65, 0)
        if 回营x == -1:
            continue
        else:
            print("回营")
